...
pub static BTC_ESPLORA_CLIENT: std::sync::LazyLock<bdk_esplora::esplora_client::AsyncClient> =
    std::sync::LazyLock::new(|| {
        bdk_esplora::esplora_client::Builder::new("https://mempool.space/signet/api")
            .build_async()
...
impl NostradeWallet {
    ...
    pub async fn sync(&self) -> Result<(), web_sys::wasm_bindgen::JsValue> {
        ...
        let full_scan_request = self
            ...
            .await
            .as_ref()
            .ok_or(wasm_bindgen::JsValue::from_str("now allet yet"))?
            .start_full_scan();
    ...
    pub async fn send_coins(
        ...
        if let Some(wallet) = btc_wallet.as_mut() {
            ...
            Err(wasm_bindgen::JsValue::from_str("Wallet not loaded"))
        }
    }
}
    ...
    let loaded = yew::suspense::use_future_with(key_ctx, |nostr_key| async move {
        if let Some(mut key) = (*nostr_key).clone() {
            ...
            if !ctx_clone.loaded() {
                ...
                ctx_clone.dispatch(NostradeWalletAction::Loaded);
                return true;
            }
            ctx_clone.loaded()
        } else {
...
pub fn use_wallet_sync() -> Option<bool> {
    let wallet_ctx = use_context::<NostradeWalletStore>().expect("No wallet context found");
    let ctx_clone = wallet_ctx.clone();
    let synced = yew::suspense::use_future_with(wallet_ctx.synced, |_synced| async move {
        if !ctx_clone.synced() {
