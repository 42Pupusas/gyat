...
impl IdbPersister {
    pub async fn find_change_set(&self) -> Result<bdk_wallet::ChangeSet, wasm_bindgen::JsValue> {
        let tx = self
            .db
            .transaction(&["change_sets"], idb::TransactionMode::ReadOnly)
            .map_err(|e| {
                wasm_bindgen::JsValue::from_str(&format!("Failed to create transaction: {e}"))
            })?;
        let store = tx.object_store("change_sets").map_err(|e| {
            wasm_bindgen::JsValue::from_str(&format!("Failed to get object store: {e}"))
        })?;

        let change_sets: Vec<wasm_bindgen::JsValue> = store
            .get_all(None, None)
            .map_err(|e| {
                wasm_bindgen::JsValue::from_str(&format!("Failed to get all change sets: {e}"))
            })?
            .await
            .map_err(|e| {
                wasm_bindgen::JsValue::from_str(&format!("Failed to await get_all: {e}"))
            })?;
        ...
        if change_sets.is_empty() {
            return Err(wasm_bindgen::JsValue::from_str("No change sets found"));
        }
    ...
    pub async fn persist_change_set(
        ...
        store
            ...
            .map_err(|e| {
                wasm_bindgen::JsValue::from_str(&format!("Failed to add change set: {e}"))
            })?;
        tx.commit().map_err(|e| {
            wasm_bindgen::JsValue::from_str(&format!("Failed to commit transaction: {e}"))
        })?;
        Ok(())
    }
